...
class Player4RemoteDataSource {
  ...
  Future<StreamingVideoInfo> getVideoInfo(
    ...
    return NetworkGuard.read(
      () async {
        try {
          // kDebugMode-gated: debugPrint survives release builds and this
          // one logs the full (signed) video URL — never ship that to
          // device logcat. Mirrors the assert-gated pattern used by
          // download_remote_ds.dart.
          if (kDebugMode) {
          ...
          if (kDebugMode) {
            debugPrint(
              '🔍 [Player4DS] video-info success: status=${response.status}',
            );
          }
          ...
          if (data is! Map<String, dynamic>) {
            if (kDebugMode) {
              debugPrint(
                '🔴 [Player4DS] Unexpected response type: ${data.runtimeType}',
              );
            }
