...
class DownloadRemoteDataSource {
  ...
  Future<CourseAccessResult> validateCourseAccess({
    ...
    try {
      ...
      if (data is! Map<String, dynamic>) {
        throw const ServerException('Invalid validate-course-access response');
      }
      ...
      throw ServerException(
        'Failed to validate course access: ${e.details ?? e.reasonPhrase ?? e.status}',
      );
    } catch (e) {
      assert(() {
  ...
  Future<VideoInfo> getVideoInfo(String url) async {
    try {
      ...
      final data = response.data;
      if (data is! Map<String, dynamic>) {
        throw const ServerException('Invalid video-info response');
      }
      ...
      throw ServerException(
        'Failed to fetch video info: ${e.details ?? e.reasonPhrase ?? e.status}',
      );
    } catch (e) {
      assert(() {
  ...
  Future<void> logDownloadAttempt({
    ...
    try {
      ...
      if (data is! Map<String, dynamic> || data['success'] != true) {
        throw const ServerException('Failed to log download attempt');
      }
    } on FunctionException catch (e) {
      throw ServerException(
        'Failed to log download attempt: ${e.details ?? e.reasonPhrase ?? e.status}',
      );
    } catch (e) {
      throw ServerException(e.toString());
    }
