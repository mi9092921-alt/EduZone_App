  ...
  Future<String> decrypt(String ciphertext) async {
    ...
    if (!ciphertext.startsWith('v')) {
      throw const FormatException(
        'Invalid encrypted format: missing version prefix', // check-ignore
      );
    }

    final colonIdx = ciphertext.indexOf(':');
    if (colonIdx == -1) {
      throw const FormatException('Invalid encrypted format'); // check-ignore
    }
    ...
    if (parts.length != 2) {
      throw const FormatException(
        'Invalid encrypted format: expected iv:ciphertext', // check-ignore
      );
    }
