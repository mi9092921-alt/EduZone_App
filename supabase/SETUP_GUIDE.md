...
### Auth Hydration (Ensures check_student_app_access() RPC works)

The `check_student_app_access()` RPC verifies:
...
```sql
-- Check that authenticated users can execute check_student_app_access()
SELECT grantee, privilege_type 
FROM information_schema.role_routine_grants
WHERE routine_name = 'check_student_app_access';
...
const { data: result, error } = await supabase.rpc('check_student_app_access');
if (error) {
...
### Issue: `[AuthProvider] check_student_app_access RPC failed: {}`
