...
- **التهيئة (2026-09-18)**: دُمجت كل ملفات البيئة في ملف `.env` واحد:
  ...
  - حُذف `.env.prod` (كان يختلف عن `.env` فقط في `APP_ENV` و`SENTRY_DSN`) —
    قيم الإنتاج بقيت كزوج معلّق داخل `.env` يُقلب عند بناء الإطلاق.
  - حُذف `.env.security.example`؛ `.env.example` أصبح القالب الوحيد المُتتبع
    ويوثّق كل المفاتيح.
  - حُدّثت كل المراجع: AGENTS.md، Makefile (وأُزيل هدف `run-staging`
    الشبحي الذي كان يشير إلى `.env.staging` غير الموجود)، `deploy.yml`
    (يكتب سر `ENV_PROD` إلى `.env` ويبني منه — يُنصح بأن يحمل السر أيضًا
    مفاتيح `SECURITY_*` ليعمل freeRASP في إصدارات الـ CI)، README الأمني،
    README.md، SECURITY.md، IMPLEMENTATION.md، وأدوات الحراسة
    (`check_config_security.py` تسمح الآن بـ `.env.example` فقط).
  - ملاحظة: `PROXY_BASE_URL` الموجود في ملفات البيئة لا يقرؤه أي كود
    حاليًا (مفتاح ميت) — بقي موثقًا في القالب حتى يُربَط أو يُحذف.
